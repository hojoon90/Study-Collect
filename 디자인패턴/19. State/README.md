...
\
Main 에 있는 setClock 메소드와 State 에 있는 doClock 메소드를 보면 Main 에서 설정한 시간을 setClock 메소드가 doClock 메소드로 위임하는 것을 볼 수 있다.
시간의 설정을 '상태에 의존한 처리'로 취급하고 있다. doClock 뿐만이 아닌 State 인터페이스에 선언된 메소드들은 모두 상태에 의존한 처리이며, 이는 모두 **상태의 동작에 따라 달라지는 처리**이다.
State 패턴에서는 상태에 의존한 처리를 **인터페이스에 추상메소드로 선언하고, 각각의 클래스에 구상메소드로 구현한다.**\
\
상태를 클래스로 표현하여 상태에 의존한 처리를 구현된 클래스에서 처리하는 것은 좋지만, 상태가 변하는 것에 대한 관리를 누가 할 것인지는 주의해야 한다.\
예제에서는 SafeFrame 클래스 안에 changeState 메소드로 상태 전환을 하였다. 하지만 이 메소드를 실제로 호출하는 클래스는 DayState 와 NightState 클래스이다.
여기에선 '상태전환'을 '상태에 의존한 처리'로 간주하고 있다. 이 방법은 각 장단점이 있는데, 아래와 같다.
* 장점
  * 다른 상태로 언제 전환하는가에 대한 정보가 하나의 클래스에 정리되어 있음.
* 단점
